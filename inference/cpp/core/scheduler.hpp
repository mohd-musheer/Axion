...
private:
    ...
    // Releases blocked by pinning or lifetime dependencies
    // are deferred here and retried on unpin / later releases,
    // instead of being silently dropped (which leaked blocks).
    std::unordered_set<std::string> deferred_releases;
